...
const configPath = path.join(__dirname, '../../moodle_config.json');
...
function build() {
    ...
    contentFiles.forEach(file => {
        ...
        if (!fileName.endsWith('.html')) {
            ...
            const previewPath = path.join(DIST_DIR, 'previews', folderName, fileName);
            ensureDir(path.dirname(distPath));
            ensureDir(path.dirname(previewPath));
            try {
                fs.copyFileSync(file, distPath);
                fs.copyFileSync(file, previewPath);
                console.log(`✅ Copied Asset: ${distPath} & ${previewPath}`);
            } catch (error) {
        ...
        // Strip outer HTML wrapper tags if present (some source files include a full HTML document structure)
        // Moodle Book only uses body content — nested <html>/<head>/<body> tags cause malformed output
        content = content
            .replace(/<!DOCTYPE[^>]*>/gi, '')
            .replace(/<html[^>]*>/gi, '')
            .replace(/<\/html>/gi, '')
